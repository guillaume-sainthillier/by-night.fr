export default class CommentApp {
    ...
    init_load_more_comments(commentaires) {
        const self = this;
        commentaires.find(self.options.css_load_more_comments).unbind("click").click(function () {
            var load_more = $(this);
    ...
    init_list_reponses(commentaire) {
        const self = this;
        commentaire.find(self.options.css_btn_list).unbind("click").click(function () {
            ...
            if (!block_reponse.hasClass(self.options.css_has_loaded_reponse)) //Les réponses ne sont pas encore chargées
            {
                ...
                icon_list
                    ...
                    .addClass(self.options.css_spinner);
            ...
            {
                if (!block_reponse.hasClass(self.options.css_has_showed_reponses)) //Les réponses ne sont pas affichées, on les affiche donc
                {
                    ...
                    block_reponse.hide(self.options.animation_duration, function () {
                        ...
                        icon_list
                            .removeClass(self.options.css_down)
                            .addClass(self.options.css_up);
                    });
                }
            }
        });
    ...
    init_load_new_reponse(commentaires) {
        const self = this;
        commentaires.find(self.options.css_link_repondre).unbind("click").click(function () //Pour tous les liens répondre
        {
            var link = $(this);

            link.data("text", link.text()).html("<i class='fa " + self.options.css_spinner + "'></i>");
            var block_post_reponse = link.closest(self.options.css_main_block_reponse).find(self.options.css_block_post_reponse); // On cherche le block du post
        ...
        $(block_post_reponse).find("form").unbind("submit").submit(function () {
            ...
            $.post(
                ...
                if (retour.success) //La réponse est envoyée
                {
                    ...
                    main_block_reponses.find(self.options.css_block_post_reponse).hide(self.options.animation_duration);
        ...
        $(commentaire).find("form").each(function () {
            ...
            $(this).unbind("submit").submit(function () {
                ...
                $.post(
                    ...
                    var block_commentaires = main_block_commentaire.find(self.options.css_block_commentaires);
                    var block_poster_commentaire = main_block_commentaire.find(self.options.css_block_poster_commentaire);
                    ...
                    if (retour.success) //Commentaire bien envoyé
                    {
                        ...
                        main_block_commentaire.find(self.options.css_heading_commentaires).replaceWith(retour.header); //On remplace le heading par le nouveau

                        var main_block_reponse = block_commentaires.find(self.options.css_block_comment).eq(0);
