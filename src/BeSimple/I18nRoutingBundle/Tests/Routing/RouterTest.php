...
class RouterTest extends \PHPUnit_Framework_TestCase
{
    public function testMatchLocaleRoute()
    {
        ...
        $parentRouter
            ...
            ->will($this->returnValue(['_route' => 'test.en', '_locale' => 'en']))
        ;
        $parentRouter
            ->expects($this->at(1))
            ->method('match')
            ->with($this->equalTo('/bar'))
            ->will($this->returnValue(['_route' => 'test.de', '_locale' => 'de']))
        ;
    ...
    public function testMatchTranslateStringField()
    {
        ...
        $parentRouter->expects($this->any())
            ...
            ->will($this->returnValue(['_route' => 'test.en', '_locale' => 'en', '_translate' => 'name', 'name' => 'beberlei']))
        ;
    ...
    public function testGenerateI18n()
    {
        ...
        $parentRouter->expects($this->once())
            ->method('generate')
            ->with($this->equalTo('test_route.en'), $this->equalTo(['foo' => 'bar']), $this->equalTo(false))
        ;
        $router = new Router($parentRouter);

        $router->generate('test_route', ['foo' => 'bar', 'locale' => 'en'], false);
    }
    ...
    public function testGenerateDefault()
    {
        ...
        $parentRouter->expects($this->once())
            ->method('generate')
            ->with($this->equalTo('test_route'), $this->equalTo(['foo' => 'bar']), $this->equalTo(false))
        ;
        $router = new Router($parentRouter);

        $router->generate('test_route', ['foo' => 'bar'], false);
    }
    ...
    public function testGenerateI18nTranslated()
    {
        ...
        $parentRouter->expects($this->once())
            ->method('generate')
            ->with($this->equalTo('test_route.en'), $this->equalTo(['foo' => 'baz']), $this->equalTo(false))
        ;
        $translator = $this->getMock('BeSimple\I18nRoutingBundle\Routing\Translator\AttributeTranslatorInterface');
        $translator
            ->expects($this->once())
        ...
        $router->generate('test_route', ['foo' => 'bar', 'translate' => 'foo', 'locale' => 'en'], false);
    }
    ...
    public function testGenerateI18nTranslatedContextLocale()
    {
        ...
        $parentRouter->expects($this->once())
            ->method('generate')
            ->with($this->equalTo('test_route.fr'), $this->equalTo(['foo' => 'baz']), $this->equalTo(false))
        ;
        $translator = $this->getMock('BeSimple\I18nRoutingBundle\Routing\Translator\AttributeTranslatorInterface');
        $translator
            ->expects($this->once())
        ...
        $router->generate('test_route', ['foo' => 'bar', 'translate' => 'foo'], false);
    }
