...
class DoctrineDBALTranslator implements AttributeTranslatorInterface
{
    ...
    public function translate($route, $locale, $attribute, $value)
    {
        ...
        if ($translatedValue = $this->connection->fetchColumn($query, [$route, $locale, $attribute, $value])) {
    ...
    public function reverseTranslate($route, $locale, $attribute, $value)
    {
        ...
        if ($lovalizedValue = $this->connection->fetchColumn($query, [$route, $locale, $attribute, $value])) {
    ...
    public function addTranslation($route, $locale, $attribute, $localizedValue, $originalValue)
    {
        ...
        $id = $this->connection->fetchColumn($query, [$route, $locale, $attribute]);

        if ($id) {
            $this->connection->update('routing_translations', [
                'localized_value' => $localizedValue,
                'original_value' => $originalValue,
            ], ['id' => $id]);
        } else {
            $this->connection->insert('routing_translations', [
                'route'  => $route,
                'locale' => $locale,
                'attribute' => $attribute,
                'localized_value' => $localizedValue,
