...
class AgendaController extends Controller
{
    ...
    {
        return $this->createForm(new AgendaType(),$agenda, [
    ...
    protected function postSocial(Agenda $agenda, $form)
    {
        ...
        {
            $want_service = $form->get("share_".$social)->getData();

            if($want_service === true)
            {
                $service = $this->get($social);
    ...
    public function newAction()
    {
        ...
        if ($this->getRequest()->isMethod('POST'))
        {
            ...
            if ($form->isValid())
            {
                ...
                $agenda->setUser($this->get('security.context')->getToken()->getUser());
//                $agenda->setId(uniqid());
                $agenda->setSite($site);
                $em->persist($agenda);
                $em->flush();
